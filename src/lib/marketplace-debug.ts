 ...
 * the last breadcrumb tells you exactly which await never returned. Copyable
 * from the dialog UI via `MarketplaceDebugCopy` so a reporting user can paste
 * console output without opening devtools.
 */
...
export function markMarketplaceDebug(label: string, data?: unknown) {
  ...
  console.debug(`[marketplace-debug] ${label}`, data ?? "");
}
