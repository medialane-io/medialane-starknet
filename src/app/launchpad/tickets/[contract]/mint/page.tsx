...
export default function MintTicketPage({ params }: { params: Promise<{ contract: string }> }) {
  ...
  const [mintedTicketId, setMintedTicketId] = useState<string | null>(null);
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      setMintError(err?.message ?? "Failed to mint tickets");
      setDialogTxStatus("idle");
      setMintStep("error");
    }
  }
  ...
  if (!isConnected) {
    return (
      <ConnectGate title="Connect wallet to mint tickets" subtitle="Connect your Starknet wallet to create and mint tickets.">
        <div />
      </ConnectGate>
    );
  }
  ...
  return (
    <>
      <ClaimRouteShell
        ...
        title="Mint Tickets"
        subtitle={`Add a new ticket type to ${collection.name ?? "your collection"} and mint its supply to your wallet, ready to list.`}
        aside={
          <>
            ...
            <ClaimRail
              included={[
                { icon: Ticket, title: "You hold the supply", desc: "Minted tickets go straight to your wallet — list them on the marketplace whenever you're ready." },
              ]}
              steps={[
                "Set an image, name, supply, and an optional validity window",
                "Mint — the full supply lands in your wallet",
                "List your tickets on the marketplace for buyers",
              ]}
              trustIcon={Ticket}
              trustLead="Your tickets, your rules."
              trust="You mint the supply and sell it your way. Holders keep their tickets forever."
            />
          </>
        }
      >
      ...
      <MintProgressDialog
        ...
        processingTitle="Minting your tickets on Starknet…"
